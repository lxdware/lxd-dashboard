...
if (!empty($_SERVER['PHP_AUTH_USER'])) {
 ...
 //Determine host info from database
  ...
  while($row = $db_results->fetchArray()){
    ...
    $exec_api_data = shell_exec("sudo curl -k -L --cert $cert --key $key -X POST -d $data $exec_url");
    ...
    $exec_operation_data = shell_exec("sudo curl -k -L --cert $cert --key $key -X GET $operation_url");
    ...
    if ($operation_data['status_code'] == 200){
      ...
      $stdout = shell_exec("sudo curl -k -L --cert $cert --key $key -X GET $stdout_log_url");
      $stderr = shell_exec("sudo curl -k -L --cert $cert --key $key -X GET $stderr_log_url");
      ...
      $stdout_delete_log = shell_exec("sudo curl -k -L --cert $cert --key $key -X DELETE $stdout_log_url");
      $stderr_delete_log = shell_exec("sudo curl -k -L --cert $cert --key $key -X DELETE $stderr_log_url");   
    }
