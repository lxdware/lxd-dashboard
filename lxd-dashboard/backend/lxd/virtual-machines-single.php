...
if (isset($_SESSION['username'])) {
  ...
  $mdev = (isset($_GET['mdev'])) ? filter_var(urldecode($_GET['mdev']), FILTER_SANITIZE_STRING) : "";
  ...
  switch ($action) {
    ...
    case "addInstanceGPUDevice":
      ...
      if (isset($data['devices'][$name])){
        ...
        if (!empty($mdev)){ $device_array['mdev'] = $mdev;}
    ...
    case "displayInstanceStateInfo":
      ...
      $processes = (isset($instance_state['processes'])) ? $instance_state['processes'] : "Unknown"; //number of process running in instance
      ...
      if ($results['status_code'] == "200"){
        ...
        foreach ($expanded_device_names as $expanded_device_name => $device_data){
          if ($device_data['type'] == "gpu"){
            ...
            $device_data_mdev = (isset($device_data['mdev'])) ? htmlentities($device_data['mdev']) : "";
    ...
    case "retrieveGaugeStats":
      ...
      $url = $base_url . "/1.0/virtual-machines/" . $instance . "/files?project=" . $project . "&path=/tmp/lxc_stats";
      $results = sendCurlRequest($action, "GET", $url);
